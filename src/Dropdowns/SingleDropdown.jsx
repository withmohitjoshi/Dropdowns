    ...
    <div className='dropdown' ref={dropdownRef}>
      ...
      {isOpen && (
        <div className='dropdown-panel'>
          ...
          {!search && Array.isArray(listData) && listData.length > 0 && listData.map(({ value, label }, index) => <DropdownListItem {...{ index, label, value, displayKeyWithLabel, onChange }} key={index} />)}
          {search && Array.isArray(listData) && listData.length > 0 && listData.filter(({ value, label }) => label.toLowerCase().includes(search.toLowerCase()) || value.includes(search)).map(({ value, label }, index) => <DropdownListItem {...{ index, label, value, displayKeyWithLabel, onChange }} key={index} />)}
        </div>
      )}
    </div>
  );
...
const DropdownListItem = ({ index, label, value, displayKeyWithLabel, onChange }) => {
  return (
    <div
      key={index}
